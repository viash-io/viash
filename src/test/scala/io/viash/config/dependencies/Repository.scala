...
class RepositoryTest extends AnyFunSuite {
  test("Repository.unapply: handles git+https syntax") {
    val repo = io.viash.config.dependencies.Repository.unapply("git+https://github.com/viash-io/viash@v1.0.0")
  ...
  test("Repository.unapply: handles github syntax") {
    val repo = io.viash.config.dependencies.Repository.unapply("github://viash-io/viash@v2.0.0")
    assert(repo.isDefined)
  ...
  test("Repository.unapply: handles viashhub syntax") {
    val repo = io.viash.config.dependencies.Repository.unapply("vsh://viash-io/viash@v2.0.0")
    assert(repo.isDefined)
  ...
  test("Repository.unapply: handles local syntax") {
    val repo = io.viash.config.dependencies.Repository.unapply("local://path/to/repo")
    assert(repo.isDefined)
    assert(repo.get.isInstanceOf[LocalRepository])
  }

  test("Repository.unapply: handles local dependency syntax") {
    val repo = io.viash.config.dependencies.Repository.unapply("local")
    assert(repo.isDefined)
    assert(repo.get.isInstanceOf[LocalRepository])
  }

  test("Repository.unapply: returns None for unrecognized syntax") {
    val repo = io.viash.config.dependencies.Repository.unapply("unknown://foo.bar")
    assert(repo.isEmpty)
  }
}
