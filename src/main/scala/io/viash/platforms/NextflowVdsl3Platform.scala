...
case class NextflowVdsl3Platform(
  ...
  | - `publish`: If `true`, the module's outputs are automatically published to `params.publishDir`.  Will throw an error if `params.publishDir` is not defined. Default is `false`.""".stripMargin)
  @example("""auto:
  |    publish: true""".stripMargin, "yaml")
