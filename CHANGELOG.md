...
* NEW FEATURE `viash ns test`: Run all tests in a particular namespace.

* NEW FEATURE: Added support for JavaScript scripts.

* NEW FEATURE: Added support for Scala scripts.

* MAJOR CHANGE: Name of autogenerated Docker image is now `ns/tool`.

* MINOR CHANGE: Internal changes to make it easier to extend viash with more scripts.

* [NXF] Default NXF image (now `ns/tool`) for consistency.
...
* TESTING `viash test`: Add tests for `viash test` functionality.
